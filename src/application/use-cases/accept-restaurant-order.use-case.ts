...
export type AcceptRestaurantOrderResult =
  | { readonly ok: true; readonly order: RestaurantOrder }
  | {
      readonly ok: false;
      readonly reason:
        | 'forbidden_or_unknown_restaurant'
        | 'unknown_order'
        | 'invalid_status'
        | 'invalid_prep_time';
    };

export class AcceptRestaurantOrderUseCase {
  constructor(
  ...
  async execute(
    ...
    if (!Number.isInteger(estimatedPrepMinutes) || estimatedPrepMinutes <= 0) {
      return { ok: false, reason: 'invalid_prep_time' };
    }
