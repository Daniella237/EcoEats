...
import { assertPreparing } from '../../domain/services/kitchen-order-policy.js';
...
export class MarkOrderReadyForPickupUseCase {
  ...
  async execute(ownerId: string, restaurantId: string, orderId: string): Promise<MarkOrderReadyResult> {
    ...
    try {
      assertPreparing(order.kitchenStatus);
    } catch {
